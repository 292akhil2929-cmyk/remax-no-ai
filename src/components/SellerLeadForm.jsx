    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className={compact ? "space-y-3" : "grid grid-cols-1 sm:grid-cols-2 gap-4"}>
        ...
        <Select value={form.property_type} onValueChange={(v) => setForm({...form, property_type: v})}>
          ...
          <SelectContent>
            {["Apartment", "Villa", "Penthouse", "Townhouse", "Land", "Office", "Retail"].map(t => (
              <SelectItem key={t} value={t}>{t}</SelectItem>
            ))}
          </SelectContent>
        </Select>
      </div>
      <Input placeholder="Property Location / Community" value={form.property_location} onChange={(e) => setForm({...form, property_location: e.target.value})} className="bg-secondary border-border/50 text-foreground placeholder:text-muted-foreground" />
      <Select value={form.reason_to_sell} onValueChange={(v) => setForm({...form, reason_to_sell: v})}>
        <SelectTrigger className="bg-secondary border-border/50 text-foreground"><SelectValue placeholder="Why are you selling/renting?" /></SelectTrigger>
        <SelectContent>
